...
const StockPriceChart = ({ tickerName, currency }: { tickerName: string, currency: string }) => {
    const [stockTimeSeriesData, setStockTimeSeriesData] = useState<StockTimeSeriesData>({
        openPrice: [],
        closePrice: [],
        dateTime: []
    });
    ...
    useEffect(() => {
        getStockGraphTimeSeriesData(tickerName)
            .then((stockData) => {
                if (stockData.data['Time Series (5min)'] === undefined) {
                    throw new Error("Stock price data not found from the API");
                }
                if (stockData.data['Time Series (5min)'].length === 0) {
                    throw new Error("Stock price data unavailable for this ticker");
                }
                ...
                for (const dt in timeSeriesData) {
                    timeSeries.openPrice.push(timeSeriesData[dt]["1. open"]);
                    timeSeries.closePrice.push(timeSeriesData[dt]["4. close"]);
                    timeSeries.dateTime.push(dt);
                }
                setStockTimeSeriesData(timeSeries);
            })
            .catch((error) => {
